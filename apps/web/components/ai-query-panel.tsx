...
export function AIQueryPanel({ databaseUrl }: AIQueryPanelProps) {
  ...
  const handleQuery = async () => {
    ...
    try {
      const response = await fetch("/api/completion", {
        ...
        body: JSON.stringify({ type: activeTab, prompt, databaseUrl }),
      });
  ...
  const renderChart = () => {
    ...
    return (
      <div className="overflow-x-auto">
        <ChartContainer
          config={{ value: { label: yAxisKey, color: "#2563eb" } }}
          className="h-[400px]"
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {!loading && result && result.success && (
          <div>{activeTab === "table" ? renderTable() : renderChart()}</div>
        )}
